...
def rdo_input():
    ...
    frame_user.pack()
    label_user = tkinter.Label(frame_user, text='【ユーザID】', width=9)
    ...
    frame_to.pack()
    label_to = tkinter.Label(frame_to, text='【To】', width=9)
    ...
    frame_cc.pack()
    label_cc = tkinter.Label(frame_cc, text='【Cc】', width=9)
    ...
    frame_bcc.pack()
    label_bcc = tkinter.Label(frame_bcc, text='【Bcc】', width=9)
    ...
    frame_addname.pack()
    label_addname = tkinter.Label(frame_addname, text='【宛名】', width=9)
    ...
    frame_name.pack()
    label_name = tkinter.Label(frame_name, text='【名前】', width=9)
    ...
    #時刻入力ボックスの配置
    frame_time = tkinter.Frame(tki)
    frame_time.pack()
    label_time1 = tkinter.Label(frame_time, text='【業務時間帯】')
    label_time1.pack(side='left')
    start_timebox = tkinter.Entry(frame_time)
    start_timebox.insert(tkinter.END, config_ini['WORK']['start_time'])
    start_timebox.pack(side='left')
    label_time2 = tkinter.Label(frame_time, text='～')
    label_time2.pack(side='left')
    finish_timebox = tkinter.Entry(frame_time)
    finish_timebox.insert(tkinter.END, config_ini['WORK']['finish_time'])
    finish_timebox.pack(side='left')

    #業務内容ボックスの配置
    frame3 = tkinter.Frame(tki)
    frame3.pack()
    label3 = tkinter.Label(frame3, text='【業務内容】', width=9)
    label3.pack(side='left')
    jobbox = tkinter.Text(frame3, width=68)
    ...
    def btn_click():
        ...
        config_ini['WORK']['start_time'] = start_timebox.get()
        config_ini['WORK']['finish_time'] = finish_timebox.get()
...
#メールを作成
    ...
    if re.fullmatch('^([1-9]|[01][0-9]|2[0-3]):[0-5][0-9]～([1-9]|[01][0-9]|2[0-3]):[0-5][0-9]$', config_ini['WORK']['start_time'] + '～' + config_ini['WORK']['finish_time']) == None:
        messagebox.showerror('エラー', '時刻が正しく入力されていません。')
        exit()
    else:
        body2 = '\n\n' + config_ini['DEFAULT']['body2'] + '\n' + config_ini['WORK']['start_time'] + '～' + config_ini['WORK']['finish_time'] + '\n\n'+ config_ini['DEFAULT']['body3'] + '\n' + config_ini['WORK']['contents'] + '\n\n'+ config_ini['DEFAULT']['body4']
    ...
    config_update.set('WORK', 'start_time', config_ini['WORK']['start_time'])
    config_update.set('WORK', 'finish_time', config_ini['WORK']['finish_time'])
